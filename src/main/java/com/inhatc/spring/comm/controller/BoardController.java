...
public class BoardController {
	...
	private String boardDetail(@PathVariable int b_no, Model model, HttpServletRequest request) throws Exception {
		...
		model.addAttribute("cop_name",mBoardService.boardDetailCopService(b_no));
		model.addAttribute("prod_name",mBoardService.boardDetailProdService(b_no));
	...
	private String boardUpdateFrom(@PathVariable int b_no, Model model) throws Exception {
		...
		//업체 전체 정보가져오기
		model.addAttribute("copList",mBoardService.copSelectAllService());
		//상품 전체 정보 가져오기
		model.addAttribute("showProd", mBoardService.showProdService());
		return "update";
	}
